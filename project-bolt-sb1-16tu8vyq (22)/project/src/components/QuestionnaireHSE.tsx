  ...
  const handleSubmit = async (values: Record<string, string>, { setSubmitting }: { setSubmitting: (isSubmitting: boolean) => void }) => {
    ...
    try {
      ...
      if (saveError) {
        ...
        throw new Error('Erro ao salvar avaliação. Por favor, tente novamente.');
      }

      setShowSuccess(true);
      
      // Wait for 2 seconds to show success message before proceeding
      ...
      setError(error instanceof Error ? error.message : 'Erro ao salvar avaliação. Por favor, tente novamente.');
    } finally {
      setLoading(false);
      setSubmitting(false);
    }
